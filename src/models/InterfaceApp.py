...
from src.utils.utils import get_weather_by_location
from src.utils.constants import JUSTIFY_CENTER
...
from src.utils.constants import FONT_POPPINS_15
from src.utils.constants import FONT_POPPINS_16
from src.utils.constants import FONT_POPPINS_20
from src.utils.constants import FONT_POPPINS_22
from src.utils.constants import FONT_POPPINS_40
from src.utils.constants import FONT_POPPINS_BOLD_25
...
class InterfaceApp:
    ...
    def _create_widgets(self) -> None:
        ...
        Entry(bg=WHITE_COLOR, width=20, font=FONT_POPPINS_BOLD_25, fg=PRIMARY_COLOR, justify=JUSTIFY_CENTER, border=0, textvariable=self._entry_place).place(x=55, y=40)
        Button(image=self._img_search_icon, border=0, bg=WHITE_COLOR, width=50, height=50, relief=RELIEF_FLAT, cursor=CURSOR_HAND2, command=lambda:self._get_weather()).place(x=410, y=32)
        ...
        Label(font=FONT_POPPINS_15, textvariable=self._label_current_weather, bg=PRIMARY_COLOR, fg=BLACK_COLOR, border=0).place(x=35,y=95)
        Label(font=FONT_POPPINS_16, textvariable=self._label_time, bg=PRIMARY_COLOR, fg=BLACK_COLOR, border=0).place(x=35,y=120)
        Label(font=FONT_POPPINS_40, textvariable=self._label_degrees, bg=PRIMARY_COLOR, fg=SECONDARY_COLOR, border=0).place(x=410,y=190)
        Label(font=FONT_POPPINS_20, textvariable=self._label_thermal_sensation, bg=PRIMARY_COLOR, fg=SECONDARY_COLOR, border=0).place(x=410,y=250)
        Label(font=FONT_POPPINS_20, textvariable=self._label_wind_text, bg=WHITE_COLOR, fg=PRIMARY_COLOR, border=0).place(x=150,y=420, anchor=ANCHOR_CENTER)
        Label(font=FONT_POPPINS_22, textvariable=self._label_wind_value, bg=WHITE_COLOR, fg=PRIMARY_COLOR, border=0).place(x=150,y=450, anchor=ANCHOR_CENTER)
        Label(font=FONT_POPPINS_20, textvariable=self._label_humidity_text, bg=WHITE_COLOR, fg=PRIMARY_COLOR, border=0).place(x=325,y=420, anchor=ANCHOR_CENTER)
        Label(font=FONT_POPPINS_22, textvariable=self._label_humidity_value, bg=WHITE_COLOR, fg=PRIMARY_COLOR, border=0).place(x=325,y=450, anchor=ANCHOR_CENTER)
        Label(font=FONT_POPPINS_20, textvariable=self._label_description_text, bg=WHITE_COLOR, fg=PRIMARY_COLOR, border=0).place(x=525,y=420, anchor=ANCHOR_CENTER)
        Label(font=FONT_POPPINS_22, textvariable=self._label_description_value, bg=WHITE_COLOR, fg=PRIMARY_COLOR, border=0).place(x=525,y=450, anchor=ANCHOR_CENTER)
        Label(font=FONT_POPPINS_20, textvariable=self._label_pressure_text, bg=WHITE_COLOR, fg=PRIMARY_COLOR, border=0).place(x=720,y=420, anchor=ANCHOR_CENTER)
        Label(font=FONT_POPPINS_22, textvariable=self._label_pressure_value, bg=WHITE_COLOR, fg=PRIMARY_COLOR, border=0).place(x=720,y=450, anchor=ANCHOR_CENTER)
    ...
    def _get_weather(self) -> None:
        ...
        weather_condtions = self._get_weather_condtions(latitude=latitude, longitude=longitude)
    ...
    def _get_weather_condtions(self, longitude: float, latitude: float) -> dict[str, int | float | str]:
        if not longitude or not latitude:
            raise ValueError("You must enter a valid longitude and latitude.")

        data = get_weather_by_location(longitude=round(longitude), latitude=round(latitude))
