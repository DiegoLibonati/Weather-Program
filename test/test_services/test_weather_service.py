...
def test_get_weather_by_location_success(
    monkeypatch: MonkeyPatch, weather_service: WeatherService
):
    fake_response = {
    ...
    result = weather_service.get_weather_by_location(longitude=10, latitude=20)

    assert "main" in result
    assert result["main"]["temp"] == 280


def test_get_weather_by_location_invalid_coordinates(weather_service: WeatherService):
    with pytest.raises(ValueError):
        weather_service.get_weather_by_location(longitude=0, latitude=None)


def test_get_place_information_success(
    monkeypatch: MonkeyPatch, weather_service: WeatherService
):
...
def test_get_place_information_location_not_found(
    monkeypatch: MonkeyPatch, weather_service: WeatherService
):
    ...
    with pytest.raises(ValueError):
        weather_service.get_place_information("UnknownPlace")
